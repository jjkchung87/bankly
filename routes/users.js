...
const { authUser, requireLogin, requireAdmin, requireThatUserOrAdmin } = require('../middleware/auth');
...
router.patch('/:username', authUser, requireLogin, requireThatUserOrAdmin, async function(
  req,
  res,
  next
) {
  try {
   ...
   //FIXES BUG #4
    if ('password' in req.body || 'username' in req.body) throw new ExpressError("Cannot update username/password", 400)
