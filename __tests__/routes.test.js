...
if (typeof TextEncoder !== 'undefined') {
  // TextEncoder is available, so you can use it
  const encoder = new TextEncoder();
  console.log(encoder)
  
  // Your code that uses TextEncoder goes here
} else {
  // TextEncoder is not available in this environment
  // You can provide a polyfill or use alternative logic here
...
describe("POST /auth/register", function() {
  ...
  //TESTS BUG #1
  test("should not allow a user to register without all required fields", async function() {
    const response = await request(app)
    .post("/auth/register")
    .send({
      username: "new_user",
  ...
  expect(response.body.message).toEqual(["instance requires property \"email\""]);
...
describe("GET /users", function() {
  ...
  test("should list all users", async function() {
    ...
    //FIX BUG #3
    expect(response.body.users).toEqual([{"username":"u1", "first_name": "fn1", "last_name":"ln1"},
...
describe("PATCH /users/[username]", function() {
  ...
  //FOR BUG #5
  test("should patch data if is current user", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new-fn1" }); // u1 is not admin
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new-fn1",
  ...
  test("should disallow patching not-allowed-fields", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, username:"new_username", admin: true });
    expect(response.statusCode).toBe(400);
  });
